 ...
 */
@RestController
@RequestMapping("/api/tPhoneVisitor")
public class ApiTPhoneVisitorController extends BaseController
{
    @Autowired
    ...
    @PostMapping
    public AjaxResult add(@RequestBody TPhoneVisitor tPhoneVisitor, HttpServletRequest request)
    {
        tPhoneVisitor.setOrigin(IpUtils.getIpAddr(request));

        if (tPhoneVisitor==null||null==tPhoneVisitor.getConnection()||
        "".equals(tPhoneVisitor.getConnection())
        ||tPhoneVisitor.getConnection().length()<4){
            return  AjaxResult.error("联系方式不能空、、、");
        }
